...
run_number=5
dir_suffix='/dinuc_shuffle/regression/'
...
label_output=Dense(1,activation='sigmoid',name='label_output')(seq_output)
...
scaled_rgs_output=Merge(mode='mul',name='scaled_rgs_output')([rgs_output,label_output])

model=Model(input=[reg_input,seq_input],output=[scaled_rgs_output,label_output])
model.compile(loss={'scaled_rgs_output':'mean_squared_error','label_output':'binary_crossentropy'},optimizer=SGD(lr=0.01,momentum=0.5),loss_weights=[1., 0.])
plot(model, show_shapes=True, to_file='%s/model.eps'%(fig_dir))

print('INFO - %s'%('loading data.'))
train,val,test=input_data.read_data_sets(train_pct=80,val_pct=10,test_pct=10,conv1d=True,add_shuffle=True)
...
history=model.fit({'seq_input':train['seq'],'reg_input':train['reg']},{'scaled_rgs_output':train['expr'],'label_output':train['real']},
	validation_data=({'seq_input':val['seq'],'reg_input':val['reg']},{'scaled_rgs_output':val['expr'],'label_output':val['real']}),
	nb_epoch=200,
	batch_size=100,
...
plt.scatter(pred[0],test['expr'])
plt.savefig("%s/pred_vs_obs.png"%(fig_dir))
output=np.column_stack((test['expr'], pred[0][:,0]))
