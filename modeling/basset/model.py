...
num_reg=472
seq_length=1000
fig_dir='../figures/basset/'
log_dir='../logs/basset/'
out_dir='../processed_data/basset/'
...
x=Convolution2D(nb_filter=filters[2],nb_row=1,nb_col=kernel_width[2],subsample=(1,1),border_mode='valid')(x)
x=BatchNormalization()(x)
x=Activation('relu')(x)
x=MaxPooling2D(pool_size=(1,4))(x)
...
model.compile(loss={'rgs_output':'mean_squared_error'},optimizer='sgd',metric='accuracy')
...
checkpoint=ModelCheckpoint(filepath="%s/model.{epoch:02d}-{val_loss:.2f}.hdf5"%(log_dir), monitor='val_loss')
