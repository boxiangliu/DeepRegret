...
def read_data_sets(seq_file='../data/yeast_promoters.txt',
	...
	target_expr_data=target_expr_data.assign(expression=categorize(target_expr_data.expression))
	...
	train_label=np.array(categorize(train['expression']))
	...
	val_label=np.array(categorize(val['expression']))
	...
	test_label=np.array(categorize(test['expression']))
