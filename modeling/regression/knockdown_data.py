...
Datasets = collections.namedtuple('Datasets', ['knockdown_data'])
...
def read_data_sets(seq_file='../data/yeast_promoters.txt',expr_file='../data/complete_dataset.txt',reg_names_file='../data/reg_names_R.txt',fold_change=0.0):
	...
	for col in range(1):
		...
		reg = reg.append(pd.DataFrame({"experiment": reg_data.columns[col], "reg_exp": [data], 'knockdown': 'WT'}))
		for i in range(len(data)):
			knockdown=data.copy()
			if fold_change==0.0:
				knockdown[i]=0.0
			else:
				knockdown[i]=knockdown[i]+fold_change
			reg = reg.append(pd.DataFrame({"experiment": reg_data.columns[col], "reg_exp": [knockdown], 'knockdown': reg_names.iloc[i,0]}))
	...
	data_meta=np.array(data_complete.loc[:,('UID','experiment','knockdown')])

	knockdown_data=DataSet(data_seq, data_reg_exp, data_label, data_meta)

	return Datasets(knockdown_data=knockdown_data)