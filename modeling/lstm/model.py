...
num_reg=472
seq_length=1000
fig_dir='../figures/lstm/model/'
log_dir='../logs/lstm/model/'
out_dir='../processed_data/lstm/model/'
...
reg_output=Reshape((1,num_reg,1))(reg_input)
...
seq_output=Reshape((length,1,filters[0]))(x)



print('INFO - %s'%('building concatenate model.'))
units=512
x=Merge(mode=lambda x: x[0] * x[1],output_shape=(length,num_reg,filters[0]))([reg_output,seq_output])
x=Reshape((length,-1))(x)
x=Bidirectional(LSTM(32,dropout_W=0.5,dropout_U=0.5))(x)
...
history=model.fit({'seq_input':train['seq'],'reg_input':train['reg']},{'rgs_output':train['expr']},validation_data=({'seq_input':val['seq'],'reg_input':val['reg']},{'rgs_output':val['expr']}),nb_epoch=1,batch_size=8,callbacks=[early_stopping,checkpoint],verbose=1)
