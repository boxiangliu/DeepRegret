...
def prediction(sess,pred_tensor,seq_placeholder,reg_expr_placeholder,labels_placeholder,keep_prob_pl,data_set):
	steps_per_epoch = data_set.num_examples // FLAGS.batch_size
	num_examples = steps_per_epoch * FLAGS.batch_size
	y=np.empty(0)
	y_=np.empty(0)
	for step in xrange(steps_per_epoch):
		feed_dict = fill_feed_dict(data_set,seq_placeholder,reg_expr_placeholder,labels_placeholder,keep_prob_pl,keep_prob=1.0,batch_size=FLAGS.batch_size)
		tmp1,tmp2 = sess.run([pred_tensor,labels_placeholder], feed_dict=feed_dict)
...
def run_training():
	...
	with tf.Graph().as_default():
		...
		pred=prediction(sess,y,seq_placeholder,reg_expr_placeholder,labels_placeholder,keep_prob_pl,data_sets.test)
